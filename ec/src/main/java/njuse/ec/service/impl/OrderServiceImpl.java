...
public class OrderServiceImpl implements OrderService {
	...
	public final ResultVo creatOrder(final int userId,
			final List<CastVo> casts, final OrderVo order) {//订单创建
		...
		int shop_id=castDao.getShopId(thisCast);
		...
		boolean success=orderDao.creatOrder(shop_id, castList,thisOrder);//订单创建是否成功
		if(success){
	...
	@Override
	public final ResultVo cancelOrder(final int userId, final OrderVo order) {//取消订单
		ResultVo result = new ResultVo();
		if(userId < 0){
			result.setResultCode(1);
	...
	@Override
	public final ResultVo payOrder(final int userId, final OrderVo order) {//订单支付
		ResultVo result = new ResultVo();
		if(userId < 0){
			result.setResultCode(1);
			result.setResultMessage("请先登录");
	...
	@Override
	public final ResultVo receiptOrder(final int userId, final OrderVo order) {//确认收货
		ResultVo result = new ResultVo();
		if(userId < 0){
			result.setResultCode(1);
			result.setResultMessage("请先登录");
	...
	@Override
	public final ResultVo refundOrder(final int userId, final OrderVo order) {//申请退款
		ResultVo result = new ResultVo();
		if(userId < 0){
			result.setResultCode(1);
			result.setResultMessage("请先登录");
	...
	@Override
	public final ResultVo refundMoney(final int userId, final OrderVo order) {//确认退款
		ResultVo result = new ResultVo();
		if(userId < 0){
			result.setResultCode(1);
			result.setResultMessage("请先登录");
	...
	@Override
	public List<OrderVo> viewOrder(int userId) {//订单查看
	...
	@Override
	public List<OrderDetailVo> viewOrderDetail(int userId, OrderVo order) {//查看订单详情
